...
class BiLSTMCRF(nn.Module):
    ...
    def __init__(self,label_size, word_embedding_size=1024, word_rnn_units=100, 
                    use_word_self_attention=False, dropout=0.5, device=torch.device('cpu'), batch_first=True):
        ...
        self.fc = nn.Linear(2*word_rnn_units, label_size)

        # self.rnn = BiLSTM(label_size, word_embedding_size=word_embedding_size, word_rnn_units=word_rnn_units, 
        #             use_word_self_attention=use_word_self_attention, dropout=dropout, use_gru=True, device=device)
        # replaced this with variational dropout based LSTM
        self.rnn = nn.LSTM(input_size=word_embedding_size, hidden_size=word_rnn_units, num_layers=1, bias=True, batch_first=self.batch_first, dropout=dropout, bidirectional=True)
        self.crf = CRF(label_size, batch_first=batch_first)

        self.softmax = nn.Softmax(dim=-1)

    def forward(self, X, y, mask):
        z, _ = self.rnn(X, mask)
        z, _ = nn.utils.rnn.pad_packed_sequence(z, batch_first=self.batch_first)
        ...
        z = self.crf(z,y,mask=mask, reduction='mean')
        return z, z_att
    ...
    def decode(self, X, mask):
        ...
        z = self.crf.decode(z, mask)
        return z, softmax_weight


class BiLSTM(nn.Module):
    def __init__(self, label_size, word_embedding_size=1024, word_rnn_units=100, 
                    use_word_self_attention=False, dropout=0.5, device=torch.device('cpu'), use_gru=True):
        ...
        self.fc = nn.Linear(2*self.word_lstm_unit, label_size)
        self.softmax = nn.Softmax(dim=-1)
...
class NERCRF(nn.Module):
    def __init__(self,in_features, out_features, batch_first=True):
        super(NERCRF, self).__init__()
        self.fc = nn.Linear(in_features, out_features)
        self.crf = CRF(out_features, batch_first=batch_first)

    def forward(self, x, y, mask):
        x = self.fc(x)

        z = self.crf(x, y, mask=mask, reduction='mean')
        return z, None

    def decode(self, x, mask):
        x = self.fc(x)

        z = self.crf.decode(x, mask)
        return z, None
