...
class SupportVectorMachineRE(RelationExtractionModel):
    ...
    def fit(self, X, y=None, max_iterations=100, C=1, sample_weight=False):
        ...
        self.svm = CalibratedClassifierCV(
            LinearSVC(max_iter=self.config.get_parameter("max_iterations"), C=self.config.get_parameter("C")))
    ...
    def transform(self, X, y=None):
        ...
        # make predictions
        log.info("Predicting relations in {} documents with SVM...".format(len(X)))
