...
from nerds.relext import bayes
from nerds.relext import logreg
from nerds.relext import svm
...
"""
RE_MODELS = {
    bayes.KEY: bayes.NaiveBayesRE,
    logreg.KEY: logreg.LogisticRegressionRE,
    svm.KEY: svm.SupportVectorMachineRE
}


class REModelEnsemble(RelationExtractionModel):
    """ Abstraction for ensembling multiple RE models and producing annotations.

        This class accepts a list of RE models as input and annotates a set of
        ...
        Attributes:
            models (list(RelExtModel)): The RE models that participate in the
                ensemble method.
    """
    ...
    def __init__(self, models):
        """
        Args:
            models: Input models must be for RE
        """
        super().__init__()
        self.models = models
        self.key = KEY
        for model in models:
            if not isinstance(model, RelationExtractionModel):
                raise ValueError("Cannot determine the ensemble type: the input models are not suitable")

    def fit(self, X, y=None, hparams=None):
        """ Train each RE model in the ensemble. The input is a list of
            `AnnotatedDocument` instances.

            Args:
                hparams (rel2bow(rel2bow)): Every model has a key associated with it
    ...
    def transform(self, X, y=None):
        ...
        relations_per_model = []
        for model in self.models:
            relations_per_model.append(model.extract(X, y))

        annotated_documents = []
        for idx, document in enumerate(X):
            # make relations
            relations = [relations_per_model[m][idx] for m in range(len(self.models))]
            relations = self.vote(np.array(relations)) if relations else []
            # make documents
    ...
    def vote(self, entity_matrix):
        """ If __k__ RE models have annotated a single document with entities,
...
class REModelEnsemblePooling(REModelEnsemble):
        ...
        """ If __k__ RE models have annotated a single document with entities,
            ...
            Args:
                annotation_matrix (2d list): Entities that have been annotated in a
                    single document by __k__ different RE models.
...
class REModelEnsembleMajorityVote(REModelEnsemble):
    def vote(self, annotation_matrix):
        """ If __k__ RE models have annotated a single document with entities,
            ...
            Example:
                ...
                been annotated by 2/3 of the RE models.
        """
        feature_set = set()
        for entity_vector in annotation_matrix:
            feature_set.update(set(entity_vector))
...
class REModelEnsembleWeightedVote(REModelEnsemble):
    def __init__(self, models):
        self.confidence_scores = []
        super().__init__(models)

    def fit(self, X, y=None, cv=5, eval_split=0.8):
        """ Train each RE model in the ensemble, and keep their cross
            validation scores to later assign a confidence level during
            voting.
        """
        for model in self.models:
            kfold = KFoldCV(model, k=cv, eval_split=eval_split, annotation_type="relation")
            hparams = {}
